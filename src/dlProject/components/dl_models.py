...
class DLModels:
    ...
    def train_model(self) -> Model:
        ...
        """ Visualize model """
        print(self.params.model_plot)
        tf.keras.utils.plot_model(self.model, self.params.model_plot, show_shapes=True)
        
        """ Compile model """
        self.model.compile(
            loss=CategoricalCrossentropy(),
            optimizer=Adam(learning_rate=self.params.model_params.learning_rate),
            metrics=['accuracy']
        )
        ...
        """ Apply batching """
        train_dataset = train_dataset.batch(self.params.model_params.train_batch_size)
        val_dataset = val_dataset.batch(self.params.model_params.test_batch_size)
        ...
        # tf.profiler.experimental.start(self.params.project.log_dir, options = options)
        # Training code here
    ...
    def _setup_callbacks(self) -> list:
        ...
        if self.params.model_params.early_stopping:
            callbacks.append(tf.keras.callbacks.EarlyStopping(
                ...
                # restore_best_weights=self.params.model_params.early_stopping.restore_best_weights
                )
            )
