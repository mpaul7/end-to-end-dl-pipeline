...
def create_train_test_dataset_tf(data_file=None, params=None, train=None, evaluation=None):
    ...
    target_column_values = params.labels.target_labels
    df_subset = df_subset[df_subset[params.labels.target_column].isin(target_column_values)]

    X = df_subset.drop(columns=[params.labels.target_column])

    _y = df_subset.loc[:, [params.labels.target_column]]
    y = pd.get_dummies(_y)
    ...
    if train:
        X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=params.validation.validation_percent)
